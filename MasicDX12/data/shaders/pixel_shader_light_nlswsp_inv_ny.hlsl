...
Texture2D gSpecularMap : TEXTURE2 : register(t3);
SamplerState SpecularMapSamplerState : SAMPLER2 : register(s3);
...
float4 main(PS_INPUT input) : SV_TARGET {
	...
	float3 specularMapSample = gSpecularMap.Sample(SpecularMapSamplerState, input.uv).rgb;
	...
	if(gDirLightCount > 0) {
		...
		litColor += texColor * (ambient + diffuse) + spec * float4(specularMapSample, 1.0f);
	}
