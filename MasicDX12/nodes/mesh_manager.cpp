...
MeshManager::MeshManager() : m_generation(0u) {}
...
uint32_t MeshManager::GetGeneration() {
	return m_generation;
}

void MeshManager::CalcInstances(const CameraNode& camera) {
	using namespace DirectX;
	for (const auto& [key, value] : m_instance_map) {
		auto& index_list = m_instance_index_map[key];
		index_list.clear();

		const auto& mesh_list = m_mesh_map[key];
...
void MeshManager::ManageInstanceBufferMap(MeshName mesh_name) {
	if (!m_instance_map.count(mesh_name)) {
		if (m_instance_buffer_map.count(mesh_name)) {
			m_instance_buffer_view_map.erase(mesh_name);
	...
	std::shared_ptr<ShaderResourceView> instance_buffer_view = device->CreateShaderResourceView(instance_buffer);
