...
void SkinnedMeshManager::AddMesh(std::shared_ptr<SceneNode> node) {
	...
	if (mesh_node) {
		...
		AnimatedMeshList::iterator iter = std::find(mesh_list.begin(), mesh_list.end(), mesh_node);
...
void SkinnedMeshManager::RemoveMesh(std::shared_ptr<SceneNode> node) {
	if (!node) return;
	std::shared_ptr<AnimatedMeshNode> mesh_node = std::dynamic_pointer_cast<AnimatedMeshNode>(node);
	if (mesh_node) {
		const std::string& name = mesh_node->Get().Name();
...
int SkinnedMeshManager::GetMeshCount(std::shared_ptr<SceneNode> node) {
	MeshName mesh_name = node->Get().Name();
	return m_mesh_map.count(mesh_name);
}

int SkinnedMeshManager::GetMeshCount(MeshName mesh_name) {
	return m_mesh_map.count(mesh_name);
}

const SkinnedMeshManager::AnimatedMeshList& SkinnedMeshManager::GetMeshList(MeshName mesh_name) const {
	return m_mesh_map.at(mesh_name);
}

const SkinnedMeshManager::AnimatedMeshMap& SkinnedMeshManager::GetMeshMap() const {
	return m_mesh_map;
}

void SkinnedMeshManager::CalcInstances(const BasicCameraNode& camera) {
	using namespace DirectX;
...
void SkinnedMeshManager::UpdateInstancesBuffer() {
	using namespace DirectX;
...
std::vector<SkinnedMeshManager::InstanceData>& SkinnedMeshManager::GetInstanceData(MeshName mesh_name) {
	return m_instance_map[mesh_name];
}

std::shared_ptr<StructuredBuffer> SkinnedMeshManager::GetInstanceBuffer(MeshName mesh_name) {
	return m_instance_buffer_map[mesh_name];
}

std::shared_ptr<ShaderResourceView> SkinnedMeshManager::GetInstanceBufferView(MeshName mesh_name) {
	return m_instance_buffer_view_map[mesh_name];
}

int SkinnedMeshManager::GetInstanceCount(MeshName mesh_name) {
	return m_instance_map.count(mesh_name);
}

SkinnedMeshManager::InstanceIndexMap& SkinnedMeshManager::GetInstanceIndexMap() {
	return m_instance_index_map;
}

std::vector<SkinnedMeshManager::InstanceIndexData>& SkinnedMeshManager::GetInstanceIndexData(MeshName mesh_name) {
	return m_instance_index_map[mesh_name];
}

int SkinnedMeshManager::GetInstanceIndexCount(MeshName mesh_name) {
	return m_instance_index_map.count(mesh_name);
}

void SkinnedMeshManager::ManageInstanceBufferMap(MeshName mesh_name) {
