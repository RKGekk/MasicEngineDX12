...
ShadowSceneVisitor::ShadowSceneVisitor(CommandList& command_list, std::shared_ptr<CameraNode> camera, EffectShadowPSO& pso, bool transparent) : m_command_list(command_list), m_camera(camera), m_shadow_pso(pso), m_transparent_pass(transparent) {
	ResetCamera();
}
...
//void ShadowSceneVisitor::ResetBoneTransforms(const std::vector<DirectX::XMFLOAT4X4>& final_transforms) {
