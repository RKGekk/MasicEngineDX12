...
void EffectShadowInstancedPSO::SetSkinnedMeshManager(std::shared_ptr<SkinnedMeshManager> skinned_mesh_manager) {
...
void EffectShadowInstancedPSO::Apply(CommandList& command_list, const GameTimerDelta& delta) {
    ...
    for (const auto& [mesh_name, mesh_node_list] : animated_mesh_map) {
        ...
        //std::shared_ptr<AnimatedMeshNode> mesh_node = mesh_list[0u];
        //const SkinnedMeshManager::AnimatedMeshList& mesh_list = mesh_node->GetMeshes();
        for (const auto& mesh_node : mesh_node_list) {
...
void EffectShadowInstancedPSO::SetFinalBoneTransforms(const std::vector<DirectX::XMFLOAT4X4>& final_transforms_matrix) {
