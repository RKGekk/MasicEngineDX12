...
EffectShadowPSO::EffectShadowPSO(std::shared_ptr<Device> device) : m_device(device), m_dirty_flags(DF_All), m_pPrevious_command_list(nullptr), m_need_transpose(true) {
    ...
    m_pipeline_state_object = m_device->CreateGraphicsPipelineState("PSOForBasicShadow"s, m_root_signature, m_vertex_shader);
