#include "effect_shadow_anim_instanced_pso.h"
...
EffectAnimShadowInstancedPSO::EffectAnimShadowInstancedPSO(std::shared_ptr<Device> device, std::shared_ptr<ShadowManager> shadow_manager) : m_device(device), m_dirty_flags(DF_All), m_pPrevious_command_list(nullptr), m_need_transpose(false), m_shadow_manager(shadow_manager) {
    ...
    root_parameters[to_underlying(RootParameters::FinalBoneTransformsData)].InitAsConstantBufferView(3, 0, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, D3D12_SHADER_VISIBILITY_ALL);
...
EffectAnimShadowInstancedPSO::~EffectAnimShadowInstancedPSO() {
    _aligned_free(m_pAligned_mvp);
    _aligned_free(m_pAligned_fbt);
}

void EffectAnimShadowInstancedPSO::SetSkinnedMeshManager(std::shared_ptr<SkinnedMeshManager> skinned_mesh_manager) {
    m_skinned_mesh_manager = skinned_mesh_manager;
}

void EffectAnimShadowInstancedPSO::Apply(CommandList& command_list, const GameTimerDelta& delta) {
    ...
    for (const auto& [mesh_name, mesh_node_list] : animated_mesh_map) {
        ...
        if (mesh_node_list.size() > 0u) {
            ...
            command_list.SetGraphicsDynamicConstantBuffer(to_underlying(RootParameters::FinalBoneTransformsData), *m_pAligned_fbt);
        }
...
void EffectAnimShadowInstancedPSO::SetFinalBoneTransforms(const std::vector<DirectX::XMFLOAT4X4>& final_transforms_matrix) {
...
void EffectAnimShadowInstancedPSO::SetViewMatrix(const ShadowCameraNode& camera) {
...
void XM_CALLCONV EffectAnimShadowInstancedPSO::SetViewMatrix(DirectX::FXMMATRIX view_matrix) {
    m_pAligned_mvp->View = view_matrix;
    m_dirty_flags |= DF_PerPassData;
}

void XM_CALLCONV EffectAnimShadowInstancedPSO::SetProjectionMatrix(DirectX::FXMMATRIX projection_matrix) {
    m_pAligned_mvp->Projection = projection_matrix;
    m_dirty_flags |= DF_PerPassData;
}

void EffectAnimShadowInstancedPSO::SetNearZ(float near_z) {
    m_near_z = near_z;
    m_dirty_flags |= DF_Near_Far;
}

void EffectAnimShadowInstancedPSO::SetFarZ(float far_z) {
    m_far_z = far_z;
    m_dirty_flags |= DF_Near_Far;
}

void EffectAnimShadowInstancedPSO::SetRenderTargetSize(DirectX::XMFLOAT2 render_target_size) {
