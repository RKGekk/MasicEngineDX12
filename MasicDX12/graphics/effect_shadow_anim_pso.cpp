#include "effect_shadow_anim_pso.h"
...
EffectShadowAnimPSO::EffectShadowAnimPSO(std::shared_ptr<Device> device, std::shared_ptr<ShadowManager> shadow_manager) : m_device(device), m_dirty_flags(DF_All), m_pPrevious_command_list(nullptr), m_need_transpose(true), m_shadow_manager(shadow_manager) {
    ...
    HRESULT hr = D3DReadFileToBlob(L"BasicShadowAnim_VS.cso", vertex_shader_blob.GetAddressOf());
    ThrowIfFailed(hr);
    m_vertex_shader = std::make_shared<VertexShader>(vertex_shader_blob, "main"s, "BasicShadowAnim_VS.cso"s);
...
EffectShadowAnimPSO::~EffectShadowAnimPSO() {
    _aligned_free(m_pAligned_mvp);
    _aligned_free(m_pAligned_fbt);
}

void EffectShadowAnimPSO::Apply(CommandList& command_list) {
...
void EffectShadowAnimPSO::SetFinalBoneTransforms(const std::vector<DirectX::XMFLOAT4X4>& final_transforms_matrix) {
...
void XM_CALLCONV EffectShadowAnimPSO::SetWorldMatrix(DirectX::FXMMATRIX world_matrix) {
    m_pAligned_mvp->World = world_matrix;
    m_dirty_flags |= DF_Matrices;
}

void XM_CALLCONV EffectShadowAnimPSO::SetViewMatrix(DirectX::FXMMATRIX view_matrix) {
    m_pAligned_mvp->View = view_matrix;
    m_dirty_flags |= DF_Matrices;
}

void XM_CALLCONV EffectShadowAnimPSO::SetProjectionMatrix(DirectX::FXMMATRIX projection_matrix) {
